...
public class DeveloperChatCommand implements CommandExecutor {

    private final StaffChat plugin;

    public DeveloperChatCommand(StaffChat plugin){
        this.plugin = plugin;
        plugin.getCommand("devchat").setExecutor(this);
    }

    @Override
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {

        if (!sender.hasPermission("staffchat.devchat") && !sender.hasPermission("staffchat.*")){
        ...
        if (args.length == 0){
            if (StaffChat.playerChatChannels.containsKey(sender)){
                if (!(StaffChat.playerChatChannels.get(sender) == ChatChannel.Dev)){
                    StaffChat.playerChatChannels.remove(sender);
                    StaffChat.playerChatChannels.put(sender, ChatChannel.Dev);
                    sender.sendMessage(Utils.chat(plugin.getConfig().getString("messages.channel-dev")));
                } else{
                    StaffChat.playerChatChannels.remove(sender);
        ...
        DeveloperChatMessageSent event = new DeveloperChatMessageSent(sender, Arrays.toString(args));
        ...
        for (Player current : Bukkit.getOnlinePlayers()){
            if (current.hasPermission("staffchat.devchat") || current.hasPermission("staffchat.*")){
                current.sendMessage(Utils.chat(plugin.getConfig().getString("chat-format.dev-chat")
